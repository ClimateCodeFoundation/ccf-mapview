#!/usr/bin/env python
# $URL: http://ccc-gistemp.googlecode.com/svn/trunk/code/step3.py $
# $Rev: 690 $
#
# step3.py
#
...
def iter_subbox_grid(station_records, max_months, first_year, radius):
    ...
    *max_months* is the maximum number of months in any station
    ...
    # ----- Jeremy
    cellid = 0 # subbox id (0-7999)
    if not os.path.exists('../data/sbx'):
        os.makedirs('../data/sbx')
    ...
    for region in regions:
        ...
        # ----- Jeremy
        box_json = {}
        box_json['coords'] = [box[0], box[2], box[1], box[3]] # [s,w,n,e]
        sbxs = [] # list of subbox indices in this box
        ...
        for subbox in subboxes:
            ...
            subbox_json = {} # ----- Jeremy
            subbox_json['coords'] = [subbox[0], subbox[2], subbox[1], subbox[3]] # [s,w,n,e]
            subbox_json['center'] = [centre[0], centre[1]] # [s/n, w/e] (haha "centre", silly British people)
            ...
            if not contributors:
                ...
                # ----- Jeremy
                subbox_json['contributors'] = []
                fout = open('../data/sbx/cell%i' % cellid + '.json', 'w')
                fout.write(str(subbox_json))
                fout.close()
                sbxs.append(cellid)
                cellid += 1
            ...
            # ----- Jeremy
            subbox_json['contributors'] = [[a[0][:len(a[0])-1],a[1]] for a in contributed] # cut off the last digit for some reason
            fout = open('../data/sbx/cell%i' % cellid + '.json', 'w')
            fout.write(str(subbox_json))
            fout.close()
            sbxs.append(cellid)
        ...
        # ----- Jeremy
        box_json['subboxes'] = sbxs
        fout = open('../data/sbx/box%i' % (regions.index(region)) + '.json', 'w')
        fout.write(str(box_json))
        fout.close()
