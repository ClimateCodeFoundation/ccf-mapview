#!/usr/bin/env python
# $URL: http://ccc-gistemp.googlecode.com/svn/trunk/code/step5.py $
# $Rev: 1008 $
#
# step5.py
#
...
def land_ocean_boxes(meta, cells):
    ...
    *cells* should be an iterator of (weight, land, ocean) triples
    ...
    def iterland():
        return land_meta, subbox_to_box(land_meta, land, celltype='LND'), land # ----- Jeremy - edited to return unboxed (only subboxes) land data

    def iterocean():
        return ocean_meta, subbox_to_box(ocean_meta, ocean, celltype='OCN'), ocean # ----- Jeremy - edited to return unboxed (only subboxes) ocean data
    ...
    def itermixed():
        ...
        return mixed_meta, subbox_to_box(mixed_meta, mixed, celltype='MIX'), mixed # ----- Jeremy - edited to return unboxed (only subboxes) mixed data
...
def step5(data):
    ...
    # ----- Jeremy
    writeMyBox(land[2], 'land')
    writeMyBox(ocean[2], 'ocean')
    writeMyBox(mixed[2], 'mixed')
    
    result = []
    for meta, boxes, subboxes in [land,ocean,mixed]: # ----- Jeremy - subboxes added (used elsewhere)
        boxes = gio.step5_bx_output(meta, boxes)
        zoned_averages = zonav(meta, boxes)
        result.append(annzon(meta, zoned_averages))
    return result

# ----- Jeremy (the whole function, it writes box data in JSON)
def writeMyBox(data, fname):
    box_sizes = {}
    if not os.path.exists('../data/temp'):
        os.makedirs('../data/temp')
    
    outdata = [[] for year in xrange(1880, 2011)]
    for d in data:
        if d.last_valid_month() > d.first_valid_month():
            months = d.last_valid_month() - d.first_valid_month() + 1
            box_sizes[months] = box_sizes.get(months, 0) + 1
        else:
            if d.last_valid_month() != 1 or d.first_valid_month() != 119988: # this is the signature of no data
                print "last: %i, first: %i" % (d.last_valid_month(), d.first_valid_month())
            box_sizes[0] = box_sizes.get(0, 0) + 1

        year_data = d.get_set_of_years(1880, 2010) #(d.first_valid_year(), d.last_valid_year())
        
        for i in xrange(len(year_data)):
            year = i + 1880
            datum = year_data[i]

            months = [a for a in datum]
            if 9999.0 not in months:
                avg = sum(months)/len(months)
            else:
                avg = 99.99 # this means no data
            outdata[i].append(avg*100)

    for year in xrange(1880, 2011):
        fout = open('../data/temp/' + fname + '.%i.grid.json' % year, 'w')
        fout.write(str(outdata[year-1880]))
        fout.close()
            
    print "Good values: %i" % sum([d.good_count for d in data])
